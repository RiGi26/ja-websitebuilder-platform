...
> **Ekstensi finansial — "operator finalisasi ongkir" (Model B, ditambah 2026-06-29):** push status boleh menyertakan field **OPSIONAL** `subtotal, ongkir, total_online, total_courier, total_gross, biaya_kurir, instruksi_bayar` (Portal `updateOrderOngkir`, dikirim saat operator set/ubah ongkir order website). Bila `ongkir` (number) hadir → WB perbarui kolom finansial + `instruksi_bayar` + set `order_projection.ongkir_status='set'`; transisi `pending→set` memicu WA `total_final` (total + rekening) ke pembeli. Bila field ini **absen** (push status biasa: verifyPayment/advance-fulfillment) → WB **tak menyentuh** finansial → **backward-compatible** (Bakso/JP tak terpengaruh). `order_projection.ongkir_status`: `'n/a'` (tenant JP/legacy — instruksi bayar langsung saat checkout) · `'pending'` (tenant ID — menunggu ongkir) · `'set'` (ongkir final). Kolom baru: `subtotal, ongkir, instruksi_bayar, ongkir_status, wa_total_sent_at` (migrasi `add_ongkir_finalize_projection.sql`).
