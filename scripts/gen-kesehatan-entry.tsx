// ============================================================
// WORKAROUND GENERATOR — sample HTML toko-kesehatan TANPA vitest/esbuild.
...
//
// Pakai:
//   npx tsc -p scripts/tsconfig.gen-kesehatan.json
//   node scripts/run-gen.cjs .tmp-gen-kesehatan .tmp-gen-kesehatan/scripts/gen-kesehatan-entry.js
//   (lalu hapus .tmp-gen-kesehatan)
//
// `npm run samples` penuh tetap jadi jalur resmi di mesin owner.
// ============================================================
import { writeFileSync, readFileSync, existsSync } from 'node:fs'
import { renderToStaticMarkup } from 'react-dom/server'
import KesehatanLuxRenderer from '../src/app/components/themes/toko-bespoke/KesehatanLuxRenderer'
...
const ENTRIES: { id: string; variant: string; label: string; sw: string }[] = [
  { id: 'kesehatan-jamu', variant: 'jamu', label: 'Toko Kesehatan — Jamu (apothecary herbal heritage)', sw: '#A8661A' },
]
...
const content = sampleContentForTheme('kesehatan-lux')
for (const e of ENTRIES) {
  const html = renderToStaticMarkup(
    <KesehatanLuxRenderer content={content} variant={e.variant} />,
  )
...
if (existsSync(idx)) {
  ...
  if (missing.length) {
    ...
    writeFileSync(idx, s)
    console.log(`ok index.html: ${missing.length} kartu kesehatan ditambahkan`)
  }
}
