// ============================================================
// WORKAROUND GENERATOR — sample HTML klinik-umum TANPA vitest/esbuild.
// (winmm.dll crash → `npm run samples` gagal di shell agen.) Jalur: tsc → node.
//
//   npx tsc -p scripts/tsconfig.gen-klinik.json
//   node scripts/run-gen.cjs .tmp-gen-klinik .tmp-gen-klinik/scripts/gen-klinik-entry.js
//   (lalu hapus .tmp-gen-klinik)
// ============================================================
import { writeFileSync, readFileSync, existsSync } from 'node:fs'
import { renderToStaticMarkup } from 'react-dom/server'
import KlinikUmumRenderer from '../src/app/components/themes/toko-bespoke/KlinikUmumRenderer'
...
const ENTRIES: { id: string; variant: string; label: string; sw: string }[] = [
  { id: 'klinik-bersih', variant: 'bersih', label: 'Klinik Umum & Gigi — Klinik Bersih (cool trust blue)', sw: '#2B5BD7' },
]
...
const content = sampleContentForTheme('klinik-lux')
for (const e of ENTRIES) {
  const html = renderToStaticMarkup(
    <KlinikUmumRenderer content={content} variant={e.variant} />,
  )
...
if (existsSync(idx)) {
  ...
  if (missing.length) {
    const cards = missing.map(
      (e) =>
        `<a class="card" href="${e.id}.html"><span class="sw" style="background:${e.sw}"></span><b>${e.label}</b><small>bespoke · ${e.id}</small><p>Renderer premium bespoke (Wave 2 jasa).</p></a>`,
    ).join('')
    s = s.replace('<div class="grid">', '<div class="grid">' + cards)
    writeFileSync(idx, s)
    console.log(`ok index.html: ${missing.length} kartu klinik ditambahkan`)
  }
}
