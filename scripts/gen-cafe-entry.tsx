// ============================================================
// WORKAROUND GENERATOR — sample HTML restaurant-cafe TANPA vitest/esbuild.
//   npx tsc -p scripts/tsconfig.gen-cafe.json
//   node scripts/run-gen.cjs .tmp-gen-cafe .tmp-gen-cafe/scripts/gen-cafe-entry.js
// ============================================================
import { writeFileSync, readFileSync, existsSync } from 'node:fs'
import { renderToStaticMarkup } from 'react-dom/server'
import CafeRenderer from '../src/app/components/themes/toko-bespoke/CafeRenderer'
...
const ENTRIES: { id: string; variant: string; label: string; sw: string }[] = [
  { id: 'cafe-seduh', variant: 'seduh', label: 'Cafe — Seduh (specialty warm-minimal, kopi-ring harga)', sw: '#A4642E' },
]
...
const content = sampleContentForTheme('cafe-lux')
for (const e of ENTRIES) {
  const html = renderToStaticMarkup(<CafeRenderer content={content} variant={e.variant} />)
...
if (existsSync(idx)) {
  ...
  if (missing.length) {
    ...
    writeFileSync(idx, s)
    console.log(`ok index.html: ${missing.length} kartu cafe ditambahkan`)
  }
}
