// ============================================================
// WORKAROUND GENERATOR — sample HTML toko-gadget TANPA vitest/esbuild.
...
//
// Pakai:
//   npx tsc -p scripts/tsconfig.gen-gadget.json
//   node scripts/run-gen.cjs .tmp-gen-gadget .tmp-gen-gadget/scripts/gen-gadget-entry.js
//   (lalu hapus .tmp-gen-gadget)
//
// `npm run samples` penuh tetap jadi jalur resmi di mesin owner.
// ============================================================
import { writeFileSync, readFileSync, existsSync } from 'node:fs'
import { renderToStaticMarkup } from 'react-dom/server'
import GadgetLuxRenderer from '../src/app/components/themes/toko-bespoke/GadgetLuxRenderer'
...
const ENTRIES: { id: string; variant: string; primary?: string; label: string; sw: string }[] = [
  { id: 'gadget-onyx', variant: 'onyx', label: 'Toko Gadget — Onyx (elektronik dark-tech)', sw: '#22D3EE' },
  { id: 'gadget-onyx-brand', variant: 'onyx', primary: '#7C3AED', label: 'Toko Gadget — Onyx (aksen brand ungu)', sw: '#7C3AED' },
]
...
const content = sampleContentForTheme('gadget-lux')
for (const e of ENTRIES) {
  const html = renderToStaticMarkup(
    <GadgetLuxRenderer content={content} variant={e.variant} primary={e.primary} />,
  )
...
if (existsSync(idx)) {
  ...
  if (missing.length) {
    ...
    writeFileSync(idx, s)
    console.log(`ok index.html: ${missing.length} kartu gadget ditambahkan`)
  }
}
