// ============================================================
// WORKAROUND GENERATOR — sample HTML klinik-fisio TANPA vitest/esbuild.
// (winmm.dll crash → `npm run samples` gagal di shell agen.) Jalur: tsc → node.
//
//   npx tsc -p scripts/tsconfig.gen-klinik.json
//   node scripts/run-gen.cjs .tmp-gen-klinik .tmp-gen-klinik/scripts/gen-klinik-fisio-entry.js
...
const ENTRIES: { id: string; variant: string; label: string; sw: string }[] = [
  { id: 'klinik-fisio', variant: 'gerak', label: 'Klinik Fisioterapi / Sport — Gerak (athletic teal + oranye)', sw: '#0E7CB0' },
]
...
for (const e of ENTRIES) {
  const html = renderToStaticMarkup(
    <KlinikFisioRenderer content={content} variant={e.variant} />,
  )
...
if (existsSync(idx)) {
  ...
  if (missing.length) {
    ...
    writeFileSync(idx, s)
    console.log(`ok index.html: ${missing.length} kartu klinik-fisio ditambahkan`)
  }
}
