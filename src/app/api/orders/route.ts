...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Tenant transferOnly (mis. Mama Zafran): alamat WAJIB (operator butuh untuk hitung
    // ongkir). Jangan percaya validasi klien saja.
    if (konfig.checkout?.transferOnly === true && !String(pembeli?.alamat ?? '').trim()) {
      return NextResponse.json({ ok: false, error: 'invalid_payload', message: 'Alamat wajib diisi.' }, { status: 400 })
    }
    ...
    if (result.status === 201) {
      ...
      after(async () => {
        ...
        // Pasar ID = ongkir menyusul → struk pending (tanpa total final); pasar JP =
        // struk normal (template tenant / default).
