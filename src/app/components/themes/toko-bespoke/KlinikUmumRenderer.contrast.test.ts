...
import { PALETTES } from './KlinikUmumRenderer'

// ============================================================
// WCAG contrast self-test — KlinikUmumRenderer KLINIK BERSIH (cool trust blue)
// AA = 4.5:1 (teks normal) | AAA = 7:1 | large/UI = 3:1
// Aksen indigo `accent` (#2B5BD7) untuk dekoratif & teks besar; teks normal
// beraksen (eyebrow/harga/role) memakai `accentDeep` (#1E44A8). Palet TUNGGAL —
// tanpa pop color (beda dari tema toko playful). Tema LIGHT → wajib gate ini.
// ============================================================
...
describe('KlinikUmumRenderer PALETTES — WCAG contrast', () => {
  for (const [name, p] of Object.entries(PALETTES)) {
    ...
    describe(`palette "${name}"`, () => {
      ...
      it('accentDeep ≥4.5 on all light surfaces (eyebrow/harga/role/teks aksen)', () => {
        for (const surf of LIGHT) {
          expect(cr(p.accentDeep, surf), `accentDeep on ${surf}`).toBeGreaterThanOrEqual(4.5)
        }
      })

      it('onAccent ≥4.5 on accentDeep (teks tombol primary/nav/panel)', () => {
