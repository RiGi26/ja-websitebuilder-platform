...
import { PALETTES } from './CafeRenderer'

// ============================================================
// WCAG contrast self-test — CafeRenderer SEDUH (specialty warm-minimal light)
// AA = 4.5:1 (teks normal) | AAA = 7:1 | large/UI = 3:1
// Aksen moka `accent` (#A4642E) hanya untuk dekoratif & teks besar (mark kutipan,
// dot nav) — teks normal beraksen (eyebrow/harga kopi-ring/angka) memakai
// `accentDeep` (#7E4A1E). Tak ada pop-fill yang membawa teks (kopi-ring = ring di
// atas surface, teks accentDeep) → tak perlu uji "ink di atas pop".
// ============================================================
...
describe('CafeRenderer PALETTES — WCAG contrast', () => {
  for (const [name, p] of Object.entries(PALETTES)) {
    ...
    describe(`palette "${name}"`, () => {
      ...
      it('accentDeep ≥4.5 on all light surfaces (eyebrow/harga kopi-ring/angka)', () => {
        for (const surf of LIGHT) {
          expect(cr(p.accentDeep, surf), `accentDeep on ${surf}`).toBeGreaterThanOrEqual(4.5)
        }
      })

      it('onAccent ≥4.5 on accentDeep (teks tombol primary/nav/CTA panel moka + aroma strip)', () => {
