    ...
    <main className={`cb-shell ${careDisplay.variable} ${careBody.variable}`}>
      ...
      <div id="care-main">
        ...
        <section className="cb-final" aria-labelledby="final-title">
          ...
          <div className="cb-final-actions">{whatsappHref ? <a className="cb-button cb-button-light" target="_blank" rel="noreferrer" href={whatsappHref}>Hubungi Admin <MessageCircle size={18} aria-hidden="true" /></a> : <button className="cb-button cb-button-light" type="button" disabled>Hubungi Admin <MessageCircle size={18} aria-hidden="true" /></button>}<a className="cb-button cb-button-outline" href="#appointment">Ajukan Jadwal <ArrowUpRight size={17} aria-hidden="true" /></a><span>{whatsappHref ? 'WhatsApp membuka pesan permintaan jadwal yang sudah disiapkan. Periksa kembali isinya sebelum mengirim.' : 'WhatsApp belum tersedia untuk preview ini. Tombol akan aktif setelah tujuan konsultasi tersedia.'}</span></div>
        </section>
      </div>
