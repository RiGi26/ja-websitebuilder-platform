...
| 2026-06-09 | **A1 + A3** | ✅ PR #105 (lanjutan) | Keputusan user: **dua-tier harga EKSPLISIT** (nol perubahan ke customer) + **triage per SKU**. `order/page.tsx` + `AddonMarketplace` konsumsi katalog (`orderAddons`/`upgradeAddons`); `CORP_TO_ORDER_ID` → `aliasToId()` (fix mapping korup blog→newsletter, track-pack→ads-tracking, g-sheets→admin). SKU `blog` (hilang) ditambah → hasBlog. Triage: drop snake-oil (protection/email-biz/client-portal) + hide heavy-unbuilt (lms/membership/cert-auto/lang-multi); 4 "wire" (delivery/newsletter/career/ads-tracking) tetap. Form order 24→18 SKU. 385 test, tsc + next build bersih. Belum: A2 (gating industri+dependency UI), merge telemedicine+live-session, B (section injection + renderer capabilities). |
