...
| 2026-06-09 | **A0** | ✅ PR #105 (CI + Vercel hijau) | SSOT `src/lib/addons/catalog.ts` (24 SKU + metadata audit) + `FLAG_ALIASES`; `addonsToFeatures` derive dari katalog via `explicitFeatures` (behavior-preserving). 40 test parity+integritas. 377 test total, tsc bersih. |
| 2026-06-09 | **A1 + A3** | ✅ PR #105 (merged) | Keputusan user: **dua-tier harga EKSPLISIT** (nol perubahan ke customer) + **triage per SKU**. `order/page.tsx` + `AddonMarketplace` konsumsi katalog (`orderAddons`/`upgradeAddons`); `CORP_TO_ORDER_ID` → `aliasToId()` (fix mapping korup blog→newsletter, track-pack→ads-tracking, g-sheets→admin). SKU `blog` (hilang) ditambah → hasBlog. Triage: drop snake-oil (protection/email-biz/client-portal) + hide heavy-unbuilt (lms/membership/cert-auto/lang-multi); 4 "wire" (delivery/newsletter/career/ads-tracking) tetap. Form order 24→18 SKU. 385 test, tsc + next build bersih. |
| 2026-06-09 | **B-cap** | ✅ PR #106 (merged) | Pipa capability: `catalog.capabilitiesForAddons()` → `BuildPlan.capabilities` (generateContent) → `konfigurasi.capabilities` (persist) → SiteRenderer pass → **RestaurantLuxRenderer baca**. UI kondisional lux: `booking`→reservasi diarahkan ke `/{slug}/booking` nyata · `delivery-buttons`→tombol "Pesan Antar" · `qr-menu`→catatan QR menu. Additive, nol regresi. 396 test (+11). Demo live terverifikasi `/restaurant-lux-test`. INSIGHT: resto+booking butuh seed service "Reservasi Meja" (route /booking gated services.length>0). |
| 2026-06-09 | **A2** | ✅ PR (gating) | Gating **dependency (hard)** + **relevansi industri (soft)** di form order. catalog: `AddonOption` +`requires`/`industries`, `midtrans.requires` DILEPAS (payment lintas shop/booking), `live-session` DISEMBUNYIKAN (orphan: induk `lms` hidden) → offered 18→17 + kurangi duplikasi telemedicine. `order/page.tsx`: `toggleAddon` cascade-deselect dependents + blok pilih bila dependency belum dipilih; kartu **locked** ("Butuh: X" + 🔒 disabled) + non-relevan **dim** ("kurang umum untuk industri"); sanitasi pre-selection kalkulator (cegah orphan). INVARIAN test: requires tiap offered add-on juga offered (no orphan-parent). 400 test (+4), tsc + next build bersih. **Audit add-on TUNTAS** (SSOT + dead-hidden + mapping-fix + dual-price + triage + gating). Belum: B-section · B-composable · merge telemedicine→"Video Meeting". |
| 2026-06-09 | **Merge-video** | ✅ PR (catalog) | **telemedicine + live-session → 1 SKU `video-meeting`** ("Integrasi Video Meeting") — selesaikan Temuan D duplikasi video. SKU baru `requires: ['booking']` (BUKAN `lms` → live-session tak lagi orphan, jadi DITAWARKAN lagi via SKU gabungan), `industries: [klinik, sekolah, personal]`, `capability: ['video-meeting']`. Id lama jadi **alias** (`aliases:['telemedicine','live-session']`) → `aliasToId`/`capabilitiesForAddons` resolve order LAMA ke video-meeting. **Parity dijaga**: `FLAG_ALIASES` +`telemedicine→[hasBooking]`, `live-session→[hasLMS,hasBooking]` shg `addonsToFeatures` order lama identik (GOLDEN tetap). `live-session` dilepas dari `NOT_OFFERED`. Offered tetap 17 (video-meeting ganti telemedicine). +5 test merge; 412 test, tsc + next build bersih. **Belum: B-section · B-composable.** |
